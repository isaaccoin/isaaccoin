...
#if defined(NDEBUG)
# error "Isaaccoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x9e2e17d913cf9a52b2b9b434c919203d15803e084cda1b82bbb9785649b3b1d7");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Isaaccoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Isaaccoin Signed Message:\n";
...
{
    // Isaaccoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 100000); // Isaaccoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 5 * 60; // Isaaccoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Isaaccoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Isaaccoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Isaaccoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb205ef04edc6f97e709e2644aa1a0a11111786e07e6fcf186e40d7d731ec5ac6");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "boris jhonson going to jail for talking shit about europe";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20  * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041cb6a10e73eb8dd0ddb5887fb5a52fb2ada41b5d6556fad8a55b1aa70a5e03091778adcce5745714907d56756b7e9e9e25f0c1074b664e8da2cc88a765aaa0f7") << OP_CHECKSIG;                                                          
        ...
        block.nVersion = 1;
        block.nTime    = 1559205719;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 109563;

        if (fTestNet)
        {
            block.nTime    = 1559205732;
            block.nNonce   = 2997742;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x4d38b98a5ec902df0acb56e7029e76127fe1ac5513d303759e390a9d621eaccf"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc2, 0xba, 0xdc }; // Isaaccoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("IsaaccoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("IsaaccoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static IsaaccoinMiner(CWallet *pwallet)
{
    printf("IsaaccoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("isaaccoin-miner");
    ...
    try { loop {
        ...
        printf("Running IsaaccoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("IsaaccoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&IsaaccoinMiner, pwallet));
}
